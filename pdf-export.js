...
async function generatePDF(puzzlesData, trimSizeStr, solutionsPerPage) {
    ...
    function drawMiniSolution(puzzleData, puzzleNum, ox, oy, boxW, boxH) {
        ...
        // ── Outer border for this slot ──────────────────────────────────
        const borderPad = 0.04;
        pdf.setDrawColor(0, 0, 0);
        pdf.setLineWidth(0.02);
        pdf.rect(ox + borderPad, oy + borderPad, boxW - 2 * borderPad, boxH - 2 * borderPad);

        // Title (solution heading, centered above grid inside border)
